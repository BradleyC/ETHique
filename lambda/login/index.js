...
// TODO: Be efficient and return twitter data after account retrieval
...
exports.handler = async (event) => {
  ...
  var handle = JSON.parse(event.body).handle;

  // Check user entry in lookup table for conditionals below.
  var storedUserMetadata = await retrieveUserMetadata(handle)
    .catch(error => {
      console.log(error);
      keyError = true;
    });
  if (keyError) return genResponse(400, 'Error checking user in lookup table');
  console.log(storedUserMetadata);
  ...
  var newAddress = await createNewAccount(handle)
    .catch(error => {
      console.log(error);
      keyError = true;
    });
